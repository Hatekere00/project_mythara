                ...
                ChatMessage(
                    ...
                    content =
                        "AUTO-REPLY MODE — you are composing a reply to ${parsed.contact} for the user, on the user's behalf, without asking the user first. They've trusted you with this contact.\n\n" +
                            ...
                            "WHEN SCREENSHOT_VIEW RETURNS A BLACK / REFUSED / FLAG_SECURE FRAME — WhatsApp / Telegram / Signal mark their chat surface FLAG_SECURE so the screenshot path returns nothing usable. The fallback (and often the BETTER path even when screenshots work) is read_recent_chat_image: it queries MediaStore for the latest image WhatsApp auto-saved to the gallery and runs THAT through the vision model. Higher resolution than a screenshot, doesn't require the chat to be open, and bypasses FLAG_SECURE entirely. Pass app_hint=\"whatsapp\" for WhatsApp specifically, or leave blank to match any messenger bucket. Use a short max_age_seconds (60-180) to make sure you're getting the image from THIS conversation, not an older one.\n\n" +
                ...
                ChatMessage(
                    ...
                    content =
                        "AUTO-TRIAGE MODE — a message arrived from someone NOT in the user's favorites. You decide whether it deserves a reply. The default answer is NO; only reply on real conversational messages from real people.\n\n" +
                            ...
                            "IMAGE UNDERSTANDING IS MANDATORY. If the screenshot_view description mentions ANY image in the conversation, you MUST understand what's IN it before replying. If the first description is vague, call screenshot_view AGAIN with focus=\"describe the photo at the bottom of the chat in detail — subject, scene\". If the screen is FLAG_SECURE and screenshots return black / refused, fall back to read_recent_chat_image with app_hint=\"whatsapp\" — this reads the auto-saved gallery image directly. Composing a reply without knowing what the image shows is unacceptable.\n\n" +
